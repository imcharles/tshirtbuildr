...
;(function($) {
	...
	var FancyProductDesigner = function( elem, args) {
		...
		$productContainer = $elem.append('<div class="fpd-product-container"><div><div class="fpd-toolbar"></div><div class="fpd-products"></div><div class="fpd-saved-products"><ul></ul></div></div><section class="fpd-clearfix"></section></div>').children('.fpd-product-container').children('div');
		...
		*/
		this.addProduct = function(views) {
			...
			$productSelection.append('<li><img src="'+views[0].thumbnail+'" title="'+views[0].title+'" /></li>');
				...
				if(params.colors.indexOf('#') == 0) {
					...
					for(var i=0; i < firstView.elements.length; i++) {
						if(firstView.elements[i].title == params.colors) {
							params.currentColor = firstView.elements[i].parameters.currentColor ? firstView.elements[i].parameters.currentColor : firstView.elements[i].parameters.colors;
						}
					}
				}
		...
		*/
		...
		var _loadProduct = function(views) {
			...
			for (var i=0; i < views.length; ++i) {
				...
				_createSingleProduct(view.title, view.elements);
				if(views.length > 1) {
		...
		var _createSingleProduct = function(title, elements) {
		...
		var _registerElementHandler = function(element) {
			...
			element.addClass('fpd-editable').children('img, canvas, p').css('cursor', 'pointer').click(function() {
				...
				//check for colors
				if(Array.isArray(elemParams.colors)) {
				...
				if($savedProductsContainer.hasClass('fpd-showing')) {
					...
					for(i in keys) {
						...
						.children('li:last').children('span').click(function() {
							_loadProduct($(this).data('elements'));
